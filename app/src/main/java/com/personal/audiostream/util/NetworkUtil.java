...
public final class NetworkUtil {
    ...
    private NetworkUtil() {
        throw new UnsupportedOperationException("NetWorkUtil实例化失败！");
    }
    ...
    public static void openWirelessSettings() {
        App.getInstance().startActivity(new Intent(android.provider.Settings.ACTION_WIRELESS_SETTINGS)
     ...
     * <p>需要异步 ping，如果 ping 不通就说明网络不可用</p>
     *
     * @param ip ip address（自己服务器 ip），如果为空，ip 为公共 ip
     * @return {@code true}: 可用<br>{@code false}: 不可用
     */
    public static boolean isAvailableByPing(String ip) {
        if (ip == null || ip.length() <= 0) {
            ip = "21.224.3.18";// todo 这里需要根据不同地区的版本手动填写  能ping通的ipaddress  此处暂时为山东邮储生产网的网址
        }
    ...
    public static boolean getWifiEnabled() {
        ...
        WifiManager manager = (WifiManager) App.getInstance().getSystemService(Context.WIFI_SERVICE);
