 ...
 * 如果调用onCreate的重载方法则使用新的audiorecord配置，酌情 Save 文件到本地
 *
 * 该类的功能：一 ：配置功能；1是否开启 Save PCM文件；2是否显示波形音量；
 *                  后面四个可能不需要了暂时：3是否显示计时；4是否自动删除文件；5是否自动上传文件；6是否暂停继续；
 *             二 ：文件：1设置 Save 文件路径及name；2 Save 文件；3获取当前 Save 文件路径；4删除文件；5 Save 成wmv格式的文件
 *             三 : 录音：1录音暂停（只有 Save 到本地的时候才有录音暂停及拼接）；2录音开始；3录音停止；4录音音频大小；
 *                        5录音长度；6录音创建及the end时间；7录音的创建人；8录音计时；9录音 send (因为是实时的，所以暂不支持录音cancel send 功能)；
 *                        10录音状态；11录音音量；
 */
public class Recorder extends JobHandler {
     ...
     * @return 1表示 Related 成功；0表示 Related 失败
     */
    ...
    /**
     * 调用该方法：则使用group name定义的AudioRecord进行录音,并创建根据 <username+年月日时分秒> 或 <时间戳> 的形式命名PCM音频源的文件
     *
     * @param audioCallBack     扩展用的接口回调
     ...
     * 调用该方法：
     * 1开启录音
     * 2是否 Save 到本地文件，setSaveFilePath()可以设置 Save 路径，但是录音文件的name格式固定：目前是pcm格式（可以转为wmv的格式）
     * 3是否开启计时（目前只记录开始时间和the end时间和时长；计时显示在界面）
     * 4是否显示波形音量
     */
    public void onStart() {
    ...
    /**
     * 调用该方法：Stop recording,并 Save 文件
     */
    public void onStop() {
        if (!(audioStatus == PAudioStatus.STATUS_START
                || audioStatus == PAudioStatus.STATUS_ERROR
        ...
        // Save 录音信息InfoBean到数据库;todo 暂时用下面的两行替代
    ...
    private void setAudioRecord(AudioRecord record) {
        if (record == null) {
            ...
            audioRecord = new AudioRecord(
                    PAudioConfig.AUDIO_SOURCE,
                    PAudioConfig.SAMPLE_RATE_IN_HZ,
                    PAudioConfig.INPUT_CHANNEL_CONFIG,
    ...
    private void setInAudioBufferSize(int bufferSize) {
        ...
        //防止因意外出现缓冲段为空的情况
        int frameSize = inAudioBufferSize / PAudioConfig.BYTES_PER_FRAME;
        ...
        if (frameSize % PAudioConfig.FRAME_COUNT != 0) {
            ...
            inAudioBufferSize = frameSize * PAudioConfig.BYTES_PER_FRAME;
        }
        Log.e(TAG, "onCreate:inAudioBufferSize== " + inAudioBufferSize);
    }
